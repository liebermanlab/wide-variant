function combine_positions_snakemake_out_vk( path_to_list_of_input_p_files, path_to_list_of_input_p_div_files, path_to_other_p_file, path_to_output_p_file, path_to_outgroup_boolean_file, REF_GENOME_DIRECTORY, looseFQmax )
...
% PLACEHOLDER
fprintf(1,'\nWARNING! Finding within-sample polymorphisms is in beta in the snakemake pipeline!!!!! -- VK\n');
fprintf(1,'\nFinding single nucleotide positions with within-sample polymorphisms...\n');

% Import list of directories for where to find variant positions for each
% sample; turn this into a cell array
fname = [ pwd '/' path_to_list_of_input_p_div_files ];
fid = fopen( fname, 'r' );
fstring = fgetl(fid); % from echo, just one line
paths_to_input_p_files = strsplit(fstring,' ') % currently printing out
fclose(fid);
...
fprintf(1,'Paths used in generate positions:') % Print for troubleshooting
paths_to_input_p_files(~in_outgroup) % Print for troubleshooting
dp = generate_positions_snakemake_vk( paths_to_input_p_files(~in_outgroup), REF_GENOME_DIRECTORY ); % Changed 2019.02.21
%cp = generate_positions_snakemake( paths_to_input_p_files, REF_GENOME_DIRECTORY ); % Changed 2019.02.21
fprintf(1,['Found ' num2str(length(dp)) ' positions where provided diversity called polymorphism in at least one sample\n']) ;
