function [an, df, mut] = annotate_mutations_auto(Positions,Call,Scaf,target_dir)

tic; fprintf(1,'annotate_mutations... ') ;
an = zeros(size(Positions,1),1) ;
for i=1:length(Scaf)
    a=Scaf{i};f=find(a=='|',2,'last');fn = a(f(1)+1:f(2)-1) ; 
    fr = fastaread([target_dir 'NCBI_Genomes\' fn '_coding_nt.fasta']) ;
    df{i} = read_gene_features(fr) ;
    z = Positions(:,1)==i ;
