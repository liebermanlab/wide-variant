function make_candidate_mutation_table_snakemake_vk( path_to_p_file, path_to_sample_names_file, path_to_outgroup_boolean_file, path_to_list_of_quals_files, path_to_list_of_diversity_files, REF_GENOME_DIRECTORY, path_to_candidate_mutation_table, path_to_coverage_matrix )
...
% path_to_coverage_matrix: where to write
% coveragematrix.mat, ex. results/coveragematrix.mat
...
% % Velina, 2020.11.11: Added back generation of coveragematrix across all positions. 
...
% Make counts -- add back all quality metrics
% for finding diverse snps instead of just first 8
counts = zeros(40, length(p), numSamples,'uint16'); % initialize
...
% point the SampleInfo to same data as above instead of samples.csv

% have modified get_all_coverage to get appropriate info 
[all_coverage_per_bp, ~, all_maf_per_bp] = get_all_coverage_vk(paths_to_diversity_files, GenomeLength);
...
fprintf(1,'WARNING! This version generates a coverage matrix.\n');
save([ pwd '/' path_to_coverage_matrix ], 'all_coverage_per_bp', '-v7.3'); 
fprintf(1,['Filename: ' pwd '/' path_to_coverage_matrix '\n']) ;
