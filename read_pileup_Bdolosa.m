function read_pileup_Bdolosa(fname_in,fname_out,ChrStarts,GenomeLength)
...
%Initialize
data=int16(zeros(numfields,GenomeLength)); %[A T C G a t c g Aq ... gq Am .... gm  At .... gt Ps Pb Pm Pftd Prtd E D]
...
while ischar(line)
    ...
    %define position, by looking at 28th character in string defining B. dolosa chrosome
    position= double(ChrStarts(int8(chr(28))-47)+sscanf(l{2},'%f'));
    ...
    for k=indelk
        ...
        calls(k:(k+1+indelsize))=-1; %don't remove base that precedes an indel
