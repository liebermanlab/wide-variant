...
def genomestats(REFGENOMEFOLDER):
    ...
    refgenome = SeqIO.parse(REFGENOMEFOLDER+'/genome.fasta','fasta')
    Genomelength = 0
    ChrStarts = []
    ScafNames = []
...
def p2chrpos(p, ChrStarts):
    ...
    if len(ChrStarts) > 1:
        ...
        positions = p - ChrStarts[chr-1] # -1 due to 0based index
        pos = np.column_stack((chr,positions))
    else:
        pos = np.column_stack((chr,p))
    return pos

def extract_outgroup_mutation_positions(REFGENOMEFOLDER,position):
    ...
    for record in refgenome:
        ...
        for sglpos in poschr:
            refnt[pos_counter] = record.seq[sglpos-1]._data # position based on p, which is 1-based coordinate
            pos_counter += 1
        chr_counter += 1
    return refnt            
...
def div_major_allele_freq(cnts):
    ...
    sorted = np.sort(c,axis=1) # return sorted matrix; axis 0 in 3d array == sort by col
    sortedpositions = np.argsort(c,axis=1) # return matrix indices of sort;axis 0 in 3d array == sort by col
    
    maxcount = sorted[:,3:4:,:] # get allele counts for major allele (4th row); weird "3:4:" indexing required to maintain 3d structure
    minorcount = sorted[:,2:3:,:] # get allele counts for first minor allele (3rd row); tri/quadro-allelic ignored; weird "2:3:" indexing required to maintain 3d structure and get 3rd row
    
    with np.errstate(divide='ignore', invalid='ignore'): # suppress warning for division by 0
        maf = maxcount / sorted.sum(axis=1,keepdims=True)
        minorAF = minorcount / sorted.sum(axis=1,keepdims=True)
    ...
    minorAF = np.squeeze(minorAF,axis=1) # leave no datat as 'nan' because 0 here could mean a pure position; axis=1 to keep 2d structure when only one position!
...
def parse_gff(REFGENOMEFOLDER,ScafNames,ortholog_info_series=pd.Series(),forceReDo=False):
    # parse gff file with genome annotation
    ...
    if os.path.isfile(REFGENOMEFOLDER+"/annotation_genes.pandas.py.pk1") and (forceReDo == False):
        ...
        gff_file = glob.glob(REFGENOMEFOLDER + '/*.gff')
        if len(gff_file) != 1:
            raise ValueError('Either no file or more than 1 *gff file found in ' + REFGENOMEFOLDER)
        examiner = GFF.GFFExaminer()
        # pre-parse file summary to check 
...
def annotate_mutations(annotation_genes , p_gp , ancnti_gp , calls_gp , counts_gp , hasmutation_gp , mutQual_gp, promotersize, ref_genome_folder):
...
#    p_gp=p[goodpos2use]
#    ancnti_gp=ancnti_m[goodpos2use,:]
...
#    mutQual_gp = mutQual[goodpos2use,].flatten() 
   # ref_genome_folder=REFGENOMEFOLDER
    ...
    for i,pos in enumerate(p_gp):
        ...
        mut_annotations['pos'] = pos_chr[i][1]
        ...
        if mut_genenum[i] == int(mut_genenum[i]): # intragenic
            ...
            mut_annotations['loc1'] = p_anno.loc['loc1']
            mut_annotations['loc2'] = p_anno.loc['loc2']
            ...
            if p_anno.loc['strand'] == 1: # get position within gene, consider strandedness
                mut_annotations['nt_pos'] = pos_chr[i][1] - p_anno.loc['loc1'] # get pos of mutation in gene. 'nt_pos' 1-based.
            elif p_anno.loc['strand'] == -1:
                mut_annotations['nt_pos'] = p_anno.loc['loc2'] - pos_chr[i][1] + 1 # +1 to fix discrepancy of loc2 0-based (but carrying index of first base after gene) and pos_chr 1-based
            else:
                mut_annotations['nt_pos'] = "." # can happen. eg. Crispr
            if p_anno.loc['strand'] == ".": # leads also to 'nt_pos' == ".". Observed with special 'type's like crispr annotated. rare! leads to no AA inference.
                aan = 9999999
            else:
                aan = int( (mut_annotations['nt_pos']-1 )/3 ) + 1 # get #codon that harbours mutation. 1-based.
                ncn = mut_annotations['nt_pos'] - ((aan-1)*3) # get #nucl within triplett
            ...
            mut_annotations['NonSyn'] = len(set(mut_annotations['AA']))>1
            ...
            if np.unique(ancnti_gp[i,:])[0] != 4: # only if anc defined; [0] ok...see next line comment
                mut_annotations['anc'] = nts[np.unique(ancnti_gp[i,:])[0]] # ancnti_gp should by definition never have > 1 unique element per row! >> [0] apropriate
            ...
            if int(mut_genenum[i])>0: # get info for gene prior SNP (if any)
                ...
                mut_annotations['distance1'] = pos_chr[i][1] - p_anno.loc['loc2']
                if p_anno.loc['strand'] == -1:
            ...
            if int(mut_genenum[i]+0.5) <= annotation_genes[p_chr-1].shape[0] and annotation_genes[p_chr-1].shape[1] !=0: # get info gene after SNP (if any); second conditional to evade empty chr
                ...
                mut_annotations['distance2'] = p_anno.loc['loc1'] - pos_chr[i][1]
                if p_anno.loc['strand'] == 1:
...
def parallel_evo_module(goodpos2use,contig_positions,annotation_mutations, annotation_genes, params_dict,plot=True):
    ...
    # % AC adjusted thresholds if looking at too many genes  --> skipped
    ...
    # dN/dS calculation
    # =============================================================================
    # calc if we observe more genes with multiple mutations than expected (simulated)
    ...
    return res_cand_nummut


def get_index_visit_sampleNames(spl_names_long):
...
def plot_molecular_clock(data,hdr,mean_count_per_visit,regress_dict):
    ...
    xpos = np.array([0,1,2,3,9]) # time in months for adding line
    ...
    ax = sns.catplot(x="Month", 
                ...
                zorder=0)    
    ax.set(ylim=(0, max(data_df['rate']+1)),xlim=(-1,10),ylabel="Number of Mutations",xlabel="Time (in months)")
