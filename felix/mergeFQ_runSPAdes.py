...
def build_sample_file_list(file):
    ...
    for line in file:
        line = line.rstrip('\n')
        ls_fwd.append("../1-data_kraken/0-tmp/"+line+"_1.fq.gz")
        ls_rev.append("../1-data_kraken/0-tmp/"+line+"_2.fq.gz")
...
def merge_fq(ls_sample_file,subject):
    ...
    outfile1 = "../1-data_kraken/0-tmp/in1_spades_"+subject+".fq.gz"
    outfile2 = "../1-data_kraken/0-tmp/in2_spades_"+subject+".fq.gz"
...
def run_spades(ls_merge_fq, subject, threads):
    ''' run spades. output to 3-spades/subject_[]/ '''
    outfolder = "3-spades/subject_"+subject+"/"
    subprocess.run("mkdir -p " + outfolder , shell=True)
    subprocess.run("/home/fkey/src/SPAdes-3.13.0-Linux/bin/spades.py --careful -t " + str(threads) + " -1 " + ls_merge_fq[0] + " -2 " + ls_merge_fq[1] + " -o " + outfolder , shell=True)
...
if __name__ == "__main__":
    ...
    run_spades(outfileLs,subjectID,threads)
    sys.exit()
