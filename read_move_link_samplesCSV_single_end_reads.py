...
def findfastqfile(dr,ID,filename):
    #NOTE: the below script is super hacky and can be consolidated. It is currently built so that the file name is assumed to be in a "fastq.gz" file. 
    #For a finalized script, this should be fixed so that ".fastq" is also an avaible single end read option
    fwd=[]
    #rev=[]
                ...
                if foldername and os.path.isdir(foldername[0]):
                    ...
                    if len(potentialhits_forward)==1: #and len(potentialhits_reverse)==1:
                        ...
                        if len(potentialhits_forward)==1: # and len(potentialhits_reverse)==1:
                            ...
                            fwd=potentialhits_forward[0]+'.gz'
                            #rev=potentialhits_reverse[0]+'.gz'
                elif foldername: # this is built for a dolosa dataset where it is known the reads are in .gz format
                    potentialhits_forward = foldername
                    fwd=potentialhits_forward[0]
                    
    if not(fwd): #or not(rev):
        raise ValueError('Single end: no file found in ' + dr + 'for ' + ID)
...
def makelink(path,sample,providername):
    ...
    [fwd_file]=findfastqfile(path,sample, providername)
